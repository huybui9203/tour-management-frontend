...
function VnPay() {
  ...
  const handlePayment = async () => {
    ...
    const { code, paymentUrl } = res.data;
  ...
  const handleTimeOut = async () => {
    try {
        await axios.delete(`${process.env.REACT_APP_URL}/order/${id}/auto-cancel`, {
            withCredentials: true
        })
        setTimeout(true);
    } catch (error) {
  ...
  return (
    <div className="max-w-full flex flex-col justify-center gap-3 py-10 text-center">
      <Dialog
        ...
        msg={"Timeout, đơn hàng của bạn đã tự động hủy"}
      />
      ...
      <div className="flex justify-center items-center gap-8 xl:gap-14 my-10">
        <div className="flex flex-col justify-center items-center gap-2">
          <div
            className={`bg-gray-400 rounded-full flex justify-center items-center w-12 h-12`}
          >
            <FormIcon />
          </div>
        ...
        <div className="flex flex-col justify-center items-center gap-2">
          <div
            className={`bg-blue-800 rounded-full flex justify-center items-center w-12 h-12`}
          >
            <PayIcon />
          </div>
      ...
      <CountdownTimer initialMinutes={1} handleTimeOut={handleTimeOut} />
      <div className="p-4 rounded">
        <h1 className="text-xl font-semibold">
          Tổng tiền cần thanh toán:{" "}
          <span className="text-2xl font-bold text-red-700">
            {formatPrice(formData?.total_price)} VNĐ
          </span>
        </h1>

        <h1 className="text-sm my-2">
          Vui lòng chọn một phương thức để thanh toán
        </h1>
        <div className="grid grid-cols-1 gap-4 mt-4">
          ...
          <div className="rounded w-full px-2 py-3 cursor-pointer flex gap-4 items-center font-semibold border border-red-500 " onClick={() => {
            navigate('/history')
          }}>
      ...
      <p className="text-sm">
        Vui lòng thanh toán đơn hàng trong 10 phút để xác nhận đơn hàng!!!
      </p>
      <p className="text-sm">
        <span className="text-red-600 font-bold">Lưu ý:</span> Sau 10 phút chưa
        thanh toán hệ thống sẽ tự động hủy đơn hàng!!
      </p>
    </div>
  );
}
