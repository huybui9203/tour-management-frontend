...
function FormBooking({ title }) {
  ...
  const promo =
    (tour?.date?.length > 0 &&
      tour.date.find((item) => item.id == dayId).promo) ||
    null;
  const priceAfterPromo = Math.floor((tour.price * (100 - promo)) / 100);
  const priceForChildren = Math.floor(
    (priceAfterPromo * (100 - PROMO_FOR_CHILD)) / 100
  );

  const fetchData = async () => {
    const res = await axios.get(
      `${process.env.REACT_APP_URL}/tour/get-details/${title}`
    );
    if (res.status !== 200) {
    ...
    setFormData((prev) => {
      ...
      return {
        ...
        adults: prev.adults.map((item) =>
          item.index === data.index && item.type === data.type
            ? { ...item, ...data }
            : item
        ),
      };
