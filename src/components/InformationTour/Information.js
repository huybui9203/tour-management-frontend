...
export default function Information({ tour, tourdayID, handleSetTourDay }) {
    ...
    const listDate = tour.date
        ? tour.date.map(
              (item) =>
                  item.start_date.slice(5, 7) + "/" + item.start_date.slice(8, 10) + "/" + item.start_date.slice(0, 4)
          )
        : [];
    ...
    const availableDays = (month) => {
        return listDate
            .filter((item) => item.slice(0, 2) == month)
            .map((item) => (item.slice(3, 5).startsWith("0") ? item.slice(4, 5) : item.slice(3, 5)));
    };
                ...
                {selectedDay ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 relative">
                            <div className="md:border-r border-gray-300 pr-4">
                                <p className="font-semibold text-black text-rose-600">
                                    Ngày đi - {selectedDay}/{selectedMonth}
                                </p>
                                <div className="flex justify-between mt-2 text-gray-600">
                                    <span>
                                        {new Date(tour.date[0]?.start_date).toTimeString().slice(0, 5) || "N/A"}
                                    </span>
                            ...
                            <div className="md:pl-4">
                                <p className="font-semibold text-black text-rose-600">
                                    Ngày về - {selectedDay}/{selectedMonth}
                                </p>
                                <div className="flex justify-between mt-2 text-gray-600">
                                    <span>{new Date(tour.date[0]?.end_date).toTimeString().slice(0, 5) || "N/A"}</span>
                                    <span>
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button className="text-teal-600 hover:text-teal-800" onClick={() => changeMonth("next")}>
                                Next &gt;
                            </button>
                        </div>
                            ...
                            {daysInSelectedMonth.map((day, index) => {
                                ...
                                return (
                                    <div
                                        ...
                                        style={{ cursor: isPast || !isAvailable ? "default" : "pointer" }} 
                                    >
                                        <div className="w-full py-2 text-lg font-semibold text-center">{day}</div>
                                        {isAvailable && (
                        ...
                        {/* Ghi chú thêm */}
                        <p className="mt-4 text-center text-red-500 italic">Vui lòng chọn ngày phù hợp</p>
                    </div>
                )}
            </div>
        </div>
