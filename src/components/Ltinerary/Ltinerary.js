...
const Ltinerary = ({ tour }) => {
    ...
    return (
        <div className="">
            {!tour?.schedules?.length && 'Đang cập nhật...'}
            {tour?.schedules?.sort((a,b) => (a.day-b.day))?.map((item) => (
                <div key={item.id} className="cursor-pointer mb-1">
                    ...
                    >
                        <span className="mr-3">{'Ngày ' +item.day } : {item.ten}</span>
                    ...
                    >
                        <p className="mb-4 pl-6 border-l-2">{item.description}</p>
                    </div>
                </div>
            ))}
        </div>
